...
export const PersonalCircumstancesPage: React.FC<{ data: SuitabilityReportData; styles: any; brand: any }> = ({
  ...
  const showDataQuality =
    data.dataQuality.mode === 'draft' && (data.dataQuality.warnings.length > 0 || data.dataQuality.missing.length > 0)
  const dataQualitySummary = [
    data.dataQuality.warnings.length > 0 ? `Warnings: ${data.dataQuality.warnings.length}` : null,
    data.dataQuality.missing.length > 0 ? `Missing items: ${data.dataQuality.missing.length}` : null
  ]
    .filter(Boolean)
    .join(' • ')

  const age = data.client.personalDetails.age
  const retirementAge = data.client.personalDetails.retirementAge
  const yearsToRetirement =
    typeof age === 'number' && typeof retirementAge === 'number' && retirementAge > age
      ? retirementAge - age
      : undefined

  const notes: string[] = []
  if (yearsToRetirement !== undefined) {
    notes.push(`Time to target retirement: approximately ${yearsToRetirement} years.`)
  }
  if (data.client.personalDetails.employmentStatus) {
    notes.push(`Employment status: ${data.client.personalDetails.employmentStatus}.`)
  }
  if (data.client.personalDetails.occupation) {
    notes.push(`Occupation: ${data.client.personalDetails.occupation}.`)
  }
  if (typeof data.client.personalDetails.dependants === 'number') {
    notes.push(`Dependants recorded: ${data.client.personalDetails.dependants}.`)
  }
  if (data.objectives.investmentTimeline) {
    notes.push(`Stated investment time horizon: ${data.objectives.investmentTimeline}.`)
  }
  if (data.objectives.incomeRequirement) {
    notes.push(`Income requirement: ${data.objectives.incomeRequirement}.`)
  }
  if (typeof data.client.financialDetails.investmentAmount === 'number') {
    notes.push(`Amount available to invest: ${formatCurrency(data.client.financialDetails.investmentAmount)}.`)
  }
  if (
    typeof data.client.financialDetails.emergencyFund === 'number' &&
    typeof data.client.financialDetails.monthlyExpenditure === 'number' &&
    data.client.financialDetails.monthlyExpenditure > 0
  ) {
    const monthsCover = data.client.financialDetails.emergencyFund / data.client.financialDetails.monthlyExpenditure
    if (Number.isFinite(monthsCover)) {
      notes.push(`Emergency fund covers approximately ${monthsCover.toFixed(1)} months of expenditure.`)
    }
  }
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        {showDataQuality && (
          <View style={styles.warningCard} wrap={false} minPresenceAhead={140}>
            <Text style={styles.subsectionTitle}>Data Quality Summary (Draft)</Text>
            <Text style={styles.text}>
              This draft report highlights missing or incomplete information. Complete the assessment before generating a
              final FCA-ready report.
            </Text>
            {dataQualitySummary ? <Text style={styles.text}>{dataQualitySummary}</Text> : null}
            {data.dataQuality.missing.length > 0 && (
              <View style={styles.bulletList}>
                {data.dataQuality.missing.slice(0, 5).map((item, idx) => (
                  <View key={idx} style={styles.bulletItem}>
                    <Text style={styles.bullet}>•</Text>
                    <Text style={styles.bulletText}>{item.message || item.key}</Text>
                  </View>
                ))}
              </View>
            )}
        ...
        <View style={styles.subsection} wrap={false} minPresenceAhead={200}>
          <Text style={styles.subsectionTitle}>Existing Arrangements Summary</Text>
          <View style={styles.card} wrap={false} minPresenceAhead={180}>
              ...
              <View style={styles.table}>
                ...
                {data.existingArrangements.pensionArrangements.length > 6 && (
                  <View style={styles.tableRow}>
                    <Text style={[styles.tableCell, { flex: 5 }]}>…and {data.existingArrangements.pensionArrangements.length - 6} more</Text>
                  </View>
                )}
              </View>
            ) : data.facts.hasPensions === true ? (
            ...
            {data.facts.hasProtection === true && data.existingArrangements.insurancePolicies?.length ? (
              <View style={styles.table}>
                ...
                {data.existingArrangements.insurancePolicies.length > 6 && (
                  <View style={styles.tableRow}>
                    <Text style={[styles.tableCell, { flex: 5 }]}>…and {data.existingArrangements.insurancePolicies.length - 6} more</Text>
                  </View>
                )}
              </View>
            ) : data.facts.hasProtection === true ? (
        ...
        <View style={styles.subsection}>
          <Text style={styles.subsectionTitle}>Key Planning Considerations</Text>
          <View style={styles.infoCard}>
            {notes.length > 0 ? (
              notes.map((note, idx) => (
              ...
              <Text style={styles.text}>Key considerations will be summarised once personal details are complete.</Text>
            )}
          </View>
        </View>
      </View>
