...
export class SuitabilityDataService {
  ...
  calculateCompletionPercentage(assessmentData: any): number {
  ...
  async autoSaveAssessment(
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .update(updatePayload)
        .eq('id', assessmentId)
        .eq('is_draft', true) // Only auto-save drafts
        .select()
        .single();
  ...
  async getLatestFinalAssessment(clientId: string) {
    try {
      ...
      console.error('Error fetching latest final assessment:', error);
      return null;
    }
  }
...
export const suitabilityDataService = new SuitabilityDataService();