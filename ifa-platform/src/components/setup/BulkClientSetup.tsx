...
const MAX_FILE_SIZE = 15 * 1024 * 1024 // 15MB - match server limit
const EXTRACT_BATCH_SIZE = 5
const ANALYSIS_POLL_INTERVAL_MS = 1500
const MAX_ANALYSIS_WAIT_MS = 3 * 60 * 1000

type ProfilePopulationStatus = 'running' | 'success' | 'failed'

interface ProfilePopulationEntry {
  status: ProfilePopulationStatus
  error?: string
}

type ReuploadStatus = 'idle' | 'uploading' | 'success' | 'failed'

interface ReuploadEntry {
  status: ReuploadStatus
  error?: string
}

function getAutoTags(fileName: string): string[] {
  const tags: string[] = []
  ...
  // Name-pattern tags (case-insensitive, word-boundary matching)
