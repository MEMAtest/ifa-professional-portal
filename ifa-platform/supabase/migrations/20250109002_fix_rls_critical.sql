-- ================================================================
-- Migration: Fix Critical RLS Issues
-- Date: 2025-01-09
-- Purpose: Address code review findings:
--   1. Add DELETE policies to all tables
--   2. Remove OR firm_id IS NULL conditions (backfill is complete)
--   3. Add error handling to JWT hook
--   4. Add RPC function to fetch user emails
--   5. Change get_my_firm_id to VOLATILE
-- ================================================================
...
BEGIN
  ...
  BEGIN
    ...
    IF NOT FOUND THEN
      ...
      -- Return event without modification - user will need to complete onboarding
      RETURN event;
    END IF;
  EXCEPTION
...
-- ================================================================
-- STEP 2: FIX get_my_firm_id TO BE VOLATILE (not cacheable)
-- ================================================================

CREATE OR REPLACE FUNCTION public.get_my_firm_id()
RETURNS UUID
LANGUAGE sql
VOLATILE  -- Changed from STABLE - auth.uid() can change
SECURITY DEFINER
SET search_path = public
AS $$
...
-- ================================================================
-- STEP 3: RPC FUNCTION TO FETCH USER EMAILS (SECURITY DEFINER)
-- ================================================================
...
REVOKE ALL ON FUNCTION public.get_firm_user_emails FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_firm_user_emails TO authenticated;

-- ================================================================
-- STEP 4: FIX CLIENTS TABLE - Remove NULL condition, ensure DELETE
-- ================================================================
...
-- ================================================================
-- STEP 5: FIX PROFILES TABLE - Add DELETE policy
-- ================================================================
...
-- ================================================================
-- STEP 14: FIX COMPLIANCE_RULES TABLE - Keep NULL for global rules
-- ================================================================
...
-- For compliance_rules, NULL firm_id means "global/system rule"
-- This is intentional - users can READ global rules but only modify their firm's rules
